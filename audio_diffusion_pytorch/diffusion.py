...
class FIRFilter(torch.nn.Module):
    ...
    def __init__(self, filter_type="hp", coef=0.85, fs=44100, ntaps=101, device=torch.device('cuda')):
        ...
        if filter_type == "hp":
            ...
            self.fir.weight.requires_grad = False
            self.fir.weight.data = torch.tensor(taps.astype("float32")).view(1, 1, -1).to(self.device)
